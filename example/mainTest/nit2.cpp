...
DWORD WINAPI nit2run(){
	...
	wait(mutex); cout<<"Nit2: Najavljeno koriscenje fajla '"<<decFPath<<"'"<<endl; signal(mutex);
	...
	//decFPath="2:\\fajl4.dat";
	decFPath[0]=p2; decFPath[7]='4';
	FS::declare(decFPath,1);
	wait(mutex); cout<<"Nit2: Najavljeno koriscenje fajla '"<<decFPath<<"'"<<endl; signal(mutex);

	//decFPath="1:\\fajl5.dat";
	decFPath[0]=p1; decFPath[7]='5';
	FS::declare(decFPath,1);
	wait(mutex); cout<<"Nit2: Najavljeno koriscenje fajla '"<<decFPath<<"'"<<endl; signal(mutex);
	...
	//3.blok
	{
		...
		wait(mutex); cout<<"Nit2: Zatvoren fajl 'fajl2.dat'"<<endl; signal(mutex);
		f2=FS::open(filepath1);
		f2->seek(f2->getFileSize());//pozicionira se na kraj fajla	
		wait(mutex); cout<<"Nit2: Otvoren fajl 'fajl2.dat' i pozicionirani smo na kraju"<<endl; signal(mutex);
		
		char filepath3[]="1:\\fajl5.dat";
		filepath3[0]=p1;
		f5=FS::open(filepath3);
		wait(mutex); cout<<"Nit2: Otvoren fajl 'fajl5.dat'"<<endl; signal(mutex);
		while(!f5->eof()){
			f5->read(1,&c);
			f2->write(1,&c);
	...
	//4.blok
	{
		...
		wait(mutex); cout<<"Nit2: Odjavljeno koriscenje fajla '"<<decFPath<<"'"<<endl; signal(mutex);
